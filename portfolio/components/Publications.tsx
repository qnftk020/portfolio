    ...
    <div className="border-b border-border last:border-b-0">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between py-4 group" aria-expanded={open}>
        <div className="flex items-center gap-4">
          ...
          <span className="font-mono text-xs text-muted">{count} paper{count > 1 ? 's' : ''}</span>
        </div>
...
export default function Publications() {
  ...
  return (
    <Section id="publications" title="Publications">
      {/* International */}
      <div className="mb-14">
        <h3 className="font-mono text-xs sm:text-sm tracking-widest uppercase text-muted mb-6 flex items-center gap-3">
          International
          <span className="text-ink/30 normal-case tracking-normal">({publicationsIntl.length})</span>
        </h3>
        <div>
          {intlByYear.map(({ year, pubs }, gi) => {
            ...
            return (
              <YearAccordion key={year} year={year} count={pubs.length} defaultOpen={gi === 0}>
                {pubs.map((pub, i) => (
                  <div key={i} className="group py-5 border-b border-border/50 last:border-b-0 hover:pl-2 sm:hover:pl-3 transition-all duration-200">
                    <div className="flex items-start justify-between gap-4 sm:gap-6">
                      <div className="flex-1 min-w-0">
                        <span className="font-mono text-[10px] text-muted/50 mr-2">[{String(startIndex - i).padStart(2, '0')}]</span>
                        <h4 className="font-serif text-base sm:text-lg font-medium text-ink leading-snug inline">{pub.title}</h4>
                        <p className="font-sans text-xs sm:text-sm text-ink/60 mt-2 leading-relaxed">
      ...
      <div>
        <h3 className="font-mono text-xs sm:text-sm tracking-widest uppercase text-muted mb-6 flex items-center gap-3">
          Domestic (Korea)
          <span className="text-ink/30 normal-case tracking-normal">({publicationsDomestic.length})</span>
        </h3>
        <div>
          {domesticByYear.map(({ year, pubs }, gi) => (
            <YearAccordion key={year} year={year} count={pubs.length} defaultOpen={gi === 0}>
              {pubs.map((pub, i) => (
                <div key={i} className="py-4 border-b border-border/50 last:border-b-0 hover:pl-2 sm:hover:pl-3 transition-all duration-200">
                  <h4 className="font-serif text-sm sm:text-base font-medium text-ink leading-snug">
                    {pub.title}
                    {pub.badge && (
