  ...
  return (
    <Link href={`/projects/${project.slug}`} className="group text-left border border-[#1F1F1D] hover:border-white/15 transition-all duration-200 block bg-[#111110]">
      <div className="aspect-square w-full bg-[#161614] overflow-hidden relative">
        {project.coverUrl ? (
          <Image src={project.coverUrl} alt={project.title} fill className="object-cover group-hover:scale-[1.02] transition-transform duration-500 opacity-90 group-hover:opacity-100" unoptimized />
        ) : (
      ...
      <div className="p-4">
        ...
        <p className="font-sans text-sm font-medium text-ink/80 leading-snug mb-1 group-hover:text-accent transition-colors">{project.title}</p>
